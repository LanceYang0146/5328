...
# --- Known transition matrices (used for forward-knownT) ---
KNOWN_T = {
...
# Build the appropriate loss function depending on the method
...
# Evaluate for one epoch (no gradient computation)
@torch.no_grad()
...
def main():
    ...
    ap.add_argument('--q', type=float, default=0.7, help='GCE q parameter in (0,1]')
    # ---- Early stopping hyperparameters ----
    ap.add_argument('--patience', type=int, default=5, help='Patience for early stopping (based on validation loss)')
    ap.add_argument('--min_delta', type=float, default=1e-4, help='Minimum improvement in validation loss to reset patience')
    args = ap.parse_args()
    ...
    if args.method == 'forward-knownT':
        ...
        # Warm-up for a few epochs using standard CE to get stable predictions
        ...
        for _ in range(3):
            ...
            for x, y in tr_loader:
                ...
                loss.backward()
                warmup_opt.step()
        # Estimate T using confident samples from validation set
    ...
    best_val_acc  = 0.0              # Best validation accuracy corresponding to best val loss
    best_state = None                # Best model state
    epochs_no_improve = 0            # Epochs since last improvement
    patience = max(0, args.patience) # 0 disables early stopping
    ...
    # ---- Compute relative reconstruction error (RRE) of estimated T ----
    T_RRE = None
    ...
    meta = {
        ...
        'best_val_acc':  best_val_acc,   #  Save best validation accuracy
        'test_acc':      ts_acc,         #  Test accuracy
        'test_loss':     ts_loss         #  Test loss
    }
    if T_tensor is not None:
        meta['T'] = (T_tensor.detach().cpu().numpy()).tolist()  #  Save estimated T
    if T_RRE is not None:
        meta['T_RRE'] = T_RRE                                  #  Save T-hat RRE
